...
public class PivotalPageLinks {
	 ...
	 * @param response
	 */
	public PivotalPageLinks(final PivotalResponse response) {
		final String linkHeader = response.getHeader(HEADER_LINK);
		if (linkHeader != null) {
			final String[] links = linkHeader.split(DELIM_LINKS);
			for (final String link : links) {
				final String[] segments = link.split(DELIM_LINK_PARAM);
				...
				for (int i = 1; i < segments.length; i++) {
					final String[] rel = segments[i].trim().split("="); //$NON-NLS-1$
